...
def crear():
    try:
        ...
        sql = "INSERT INTO PASAJES (id_ruta, id_unidad, id_tipo, valor_pasaje, fecha_viaje) VALUES (:1, :2, :3, :4, SYSDATE)"
        cursor.execute(sql, [id_ruta, id_unidad, id_tipo, valor])
        conn.commit()
        conn.close()
    ...
    if request.method == 'POST':
        try:
            ...
            sql = """UPDATE PASAJES SET id_ruta=:1, id_unidad=:2, id_tipo=:3, valor_pasaje=:4 
                     WHERE id_pasaje=:5"""
            cursor.execute(sql, [id_ruta, id_unidad, id_tipo, valor, id])
            conn.commit()
        except Exception as e:
        ...
        conn.close()
        return render_template('editar_pasaje.html', pasaje=pasaje, rutas=rutas, unidades=unidades, tipos=tipos)
